<?php 
/*
Template Name:  Detail Speciale dag
*/
get_header(); 
			...
			if($_POST['addMid']){
				...
				if(empty($_POST['beschrijving'])){	$error .= "<li>Je bent vergeten een beschrijving in te vullen</li>";}
					...
					//Campagne updaten
						...
						$wpdb->update( Special, array( 'Titel' => $_POST['titel'], 'Tijd_van' => $startD, 'Tijd_tot' => $eindD, 'Omschrijving' => $_POST['beschrijving'] ),array( 'ID' => $_GET['id']) ); 
	...
	<div id="contentLinks">
		...
		<div class="contentHeaderIcon"><img src="<?php bloginfo('template_url'); ?>/images/iconSpecial.png" width="30"></div>
			<h1>Speciale dag(en): <?php echo $actDetails->Titel; ?></h1>
			<div class="edit"><?php if(!isset($_GET['edit'])){ ?><a href="/sur/contentdetail-speciale-dagen/?id=<?php echo $actDetails->ID; ?>&edit=1" ><img src="<?php bloginfo('template_url'); ?>/images/edit.png" alt="" title="Bewerk de gegevens" /></a><?php } ?><a href="/sur/contentkalender?delete2=yes&id=<?php echo $actDetails->ID; ?>" ><img src="<?php bloginfo('template_url'); ?>/images/iconDelete.png" alt="" title="Verwijderen" onclick="return confirm('Weet je zeker dat je deze speciale dag(en) wilt verwijderen?')"/></a></div>
		</div>
		...
		?>
			...
			?>
				...
				<form action="https://www.expect-webmedia.nl/sur/contentdetail-speciale-dagen/?id=<?php echo $_GET['id'];?>&show=1 " method="post">
				Titel *<br />
	...
	<?php if($actDetails->type == 'Campagne'){ ?>
		...
		<?php 
				global $wpdb;
				$vandaag = date('Ymd');
				$activiteiten = $wpdb->get_results( "SELECT * FROM Content WHERE Special = '".$actDetails->ID."'" );
				?>
		...
		<div class="contentContentdr">
			...
			<?php
				...
				<tr>
					<td valign="top"><a href="/sur/contentdetail/?id=<?php echo $activiteit->ID; ?>"><?php echo $activiteit->type; ?></a></td>
					<td valign="top" width="10">:</td>
					<td valign="top" width="300"><a href="/sur/contentdetail/?id=<?php echo $activiteit->ID; ?>"><?php echo $activiteit->Titel; ?></a></td>
				</tr>
				<?php }
